...
module.exports = async (req, res) => {
  ...
  if (!itemId || typeof req.body?.active !== "boolean") {
    res.status(400).json({
      error: {
        code: "BAD_REQUEST",
        message: "Missing itemId or active.",
      },
    });
    return;
  }
  ...
  try {
    ...
    res.status(Number(error?.status) || 502).json({
      error: {
        code: String(error?.code || "ACTIVE_PATCH_FAILED"),
        message: error?.message || "Could not update recruitment active status.",
      },
    });
  }
};
