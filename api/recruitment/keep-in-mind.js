...
module.exports = async (req, res) => {
  ...
  if (!itemId || typeof req.body?.keepInMind !== "boolean") {
    res.status(400).json({
      error: {
        code: "BAD_REQUEST",
        message: "Missing itemId or keepInMind.",
      },
    });
    return;
  }
  ...
  try {
    ...
    res.status(Number(error?.status) || 502).json({
      error: {
        code: String(error?.code || "KEEP_IN_MIND_PATCH_FAILED"),
        message: error?.message || "Could not update keep in mind.",
      },
    });
  }
};
