...
const recruitmentCreateHandler = require("./api/recruitment/create");
...
const tasksTaskSetsHandler = require("./api/tasks/task-sets");
...
async function handleApi(req, res, reqUrl) {
  ...
  if (reqUrl.pathname === "/api/recruitment/create") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
    await recruitmentCreateHandler(apiReq, apiRes);
    return true;
  }
  ...
  if (reqUrl.pathname === "/api/recruitment/initial-screen") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
    await recruitmentInitialScreenHandler(apiReq, apiRes);
    return true;
  }

  if (reqUrl.pathname === "/api/recruitment/stage") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
    await recruitmentStageHandler(apiReq, apiRes);
    return true;
  }
  ...
  if (reqUrl.pathname === "/api/recruitment/owner") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
  ...
  if (reqUrl.pathname === "/api/recruitment/details") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
  ...
  if (reqUrl.pathname === "/api/cron/enquiry-reminders") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
  ...
  if (reqUrl.pathname === "/api/tasks/task-sets") {
    await tasksTaskSetsHandler(apiReq, apiRes);
    return true;
  }

  if (reqUrl.pathname === "/api/tasks/create-batch") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
    await tasksCreateBatchHandler(apiReq, apiRes);
    return true;
  }

  if (reqUrl.pathname === "/api/maps/drive-time") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
  ...
  if (reqUrl.pathname === "/api/maps/office-catchment/check-click") {
    if (req.method === "POST") {
      apiReq.body = await readJsonBody(req);
    }
