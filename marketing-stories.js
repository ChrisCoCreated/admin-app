...
function redirectToUnauthorized(pageKey) {
  const page = encodeURIComponent(String(pageKey || "marketingstories").trim().toLowerCase());
...
async function init() {
  try {
    ...
    if (!canAccessPage(role, "marketingstories")) {
      redirectToUnauthorized("marketingstories");
      return;
    }
