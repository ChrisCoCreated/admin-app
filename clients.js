...
const VALID_CLIENT_AREA_BY_KEY = new Map(VALID_CLIENT_AREAS.map((area) => [area.toLowerCase(), area]));
...
function getClientArea(client) {
  const area = String(client?.area || "").trim().toLowerCase();
  return VALID_CLIENT_AREA_BY_KEY.get(area) || "-";
}
...
function renderAreaFilter() {
  ...
  const availableAreas = new Set(
    allClients
      .map((client) => VALID_CLIENT_AREA_BY_KEY.get(String(client?.area || "").trim().toLowerCase()))
      .filter(Boolean)
  );
  const options = VALID_CLIENT_AREAS.filter((area) => availableAreas.has(area));
